                     ...
                     int trajectory_downsample) const
{
    ...
    const double denom = std::cos(tool.gama_f) - std::sin(tool.gama_f);
    if (std::abs(denom) < 1e-9)
        throw std::runtime_error("Denominator too small in L computation");

    const double L_min = tool.fz / (2.0 * denom);
    constexpr double eps = 1e-6;
    const double L = L_min + eps;

    const int grid_size = (m + 1) * (n + 1);
    const int z_n = tool.z_n;

    // -----------------------------
    // Precompute tool edge points
    // -----------------------------
    std::vector<Eigen::Vector4d> tool_points(edge_points);
    for (int i = 0; i < edge_points; ++i)
    {
        double l = -L + 2.0 * L * i / (edge_points - 1);
        double z = R - std::sqrt(std::max(0.0, R*R - l*l));
    ...
    // =====================================================
    // SURFACE SIMULATION (atomic grid updates, parallel)
    // =====================================================
    std::vector<std::atomic<double>> Z(grid_size);
    for (int i = 0; i < grid_size; ++i)
        Z[i].store(z_level, std::memory_order_relaxed);
    ...
    #pragma omp parallel
    {
        ...
        for (int t_idx = 0; t_idx < num_time_steps; ++t_idx)
        {
            ...
            // Single atomic update per touched cell
            for (int idx : touched_cells)
            {
                double zp = local_min[idx];
                double old_val = Z[idx].load(std::memory_order_relaxed);
    ...
    // =====================================================
    // Convert grid to surface points (parallel)
    // =====================================================
    surface_points.resize(grid_size);
    ...
    for (int i = 0; i <= m; ++i)
    {
        ...
        {
            int idx = i * (n + 1) + j;
            surface_points[idx] = Eigen::Vector3d(i * delta_x, j * delta_y, Z[idx].load(std::memory_order_relaxed));
        }
    }

    // =====================================================
    // TRAJECTORY COMPUTATION (separate, optional, downsampled)
    // =====================================================
    trajectory_points.clear();
    if (trajectory_downsample > 0)
    {
        trajectory_points.reserve((num_time_steps / trajectory_downsample + 1) * z_n);

        #pragma omp parallel
        {
            std::vector<TrajectoryPoint> traj_local;
            traj_local.reserve(z_n);

            #pragma omp for schedule(dynamic)
            for (int t_idx = 0; t_idx < num_time_steps; t_idx += trajectory_downsample)
            {
                double t = t_idx * delta_t;
                traj_local.clear();
                ...
                for (int K = 1; K <= z_n; ++K)
                {
                    ...
                    traj_local.push_back({K, t, min_point(0), min_point(1), min_point(2)});
                }

                #pragma omp critical
                trajectory_points.insert(trajectory_points.end(), traj_local.begin(), traj_local.end());
            }
        }
    }
} */
